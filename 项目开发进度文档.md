...
- ✅ Redis 缓存
- ✅ Vue 3 前端界面
- ✅ 本地图片路径自动处理（支持 Typora 等编辑器粘贴的本地图片）
...
- **文件上传**: 图片、头像、封面
- **Redis缓存**: 文章列表、详情缓存
- **本地图片自动处理**: 粘贴 Typora 等编辑器的 Markdown 内容时，自动将本地图片路径（如 `/Users/.../image.png`）复制到服务器并替换为可访问的 URL
...
#### v5.1 (2025-12-01)
- 移除新建文章时的手动上传图片按钮
- 支持粘贴带本地图片路径的 Markdown 内容，保存时自动处理本地图片
- 优化代码结构
